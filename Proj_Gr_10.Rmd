...
# Josie's new stuff
```{r}
# Create a data frame that will have the info from team_record_df and the info 
# from cfbstats_df (with each stat as its own column)
team_record_df_expanded <- team_record_df
team_record_df_expanded["rushingAttempts"] <- NA
team_record_df_expanded["interceptionTDs"] <- NA
team_record_df_expanded["passCompletions"] <- NA
team_record_df_expanded["interceptions"] <- NA
team_record_df_expanded["tacklesForLoss"] <- NA
team_record_df_expanded["rushingYards"] <- NA
team_record_df_expanded["penaltyYards"] <- NA
team_record_df_expanded["kickReturnYards"] <- NA
team_record_df_expanded["kickReturnTDs"] <- NA
team_record_df_expanded["totalYards"] <- NA
team_record_df_expanded["fumblesRecovered"] <- NA
team_record_df_expanded["fumblesLost"] <- NA
team_record_df_expanded["games"] <- NA
team_record_df_expanded["puntReturns"] <- NA
team_record_df_expanded["penalties"] <- NA
team_record_df_expanded["fourthDowns"] <- NA
team_record_df_expanded["rushingTDs"] <- NA
team_record_df_expanded["netPassingYards"] <- NA
team_record_df_expanded["turnovers"] <- NA
team_record_df_expanded["possessionTime"] <- NA
team_record_df_expanded["thirdDownConversions"] <- NA
team_record_df_expanded["puntReturnTDs"] <- NA
team_record_df_expanded["sacks"] <- NA
team_record_df_expanded["interceptionYards"] <- NA
team_record_df_expanded["passAttempts"] <- NA
team_record_df_expanded["firstDowns"] <- NA
team_record_df_expanded["puntReturnYards"] <- NA
team_record_df_expanded["passingTDs"] <- NA
team_record_df_expanded["fourthDownConversions"] <- NA
team_record_df_expanded["passesIntercepted"] <- NA
team_record_df_expanded["kickReturns"] <- NA
team_record_df_expanded["thirdDowns"] <- NA

# Iterate through cfbstats_df and fill in the appropriate row with that stat's 
# information (matching based on team and year)
for (i in 1:nrow(cfbstats_df))
{
    tmp <- cfbstats_df[i,]
    team_record_df_expanded[team_record_df_expanded$team == tmp$team & 
        team_record_df_expanded$year == tmp$season,][grep(tmp$statName, 
        names(team_record_df_expanded))] <- tmp$statValue
}

# Create a simple linear regression based on all of the statistics, on total wins
linear_regression = lm(total.wins ~ firstDowns + 
                ...
                thirdDowns +
                totalYards +
                turnovers, data = team_record_df_expanded)

# Analyze the regression - summary shows good details on the 
...
# TODO: Pending tasks:
# 1. change the coloring theme for the unselected states to be in grey
# 1a. change the legend title for the plot - if possible implement using the 
# qplot()
# 2. create a multiple plot for each years - just to get a feel of how
# teams are doing over the years
# 3. Creation of a dashboard consolidation every statistics into a single plot
# 4. sort of a tornado plot? cylindrical histogram?
# 5. Adjustment to the function and opportunity to create a 
# new function by itself
...
# show the list of states that are invovled in the Big 10 Conferences
